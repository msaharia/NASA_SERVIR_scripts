...
CLIMDIR= str(sys.argv[17])
FCSTRAWDIR= str(sys.argv[18])
...
for MON in [INIT_FCST_MON]:
	...
	FCST_COARSE = np.empty(((TARGET_FCST_EYR-TARGET_FCST_SYR)+1, LEAD_FINAL, ENS_NUM, len(LATS), len(LONS))) 
...
#					print (INFILE)
	...
	CORRECT_FCST_COARSE = np.ones(((TARGET_FCST_EYR-TARGET_FCST_SYR)+1, LEAD_FINAL, ENS_NUM, len(LATS), len(LONS)))*-999
...
#				print (time.time()-start_time)
	...
	write_4d_netcdf(OUTFILE, CORRECT_FCST_COARSE, OBS_VAR, MODEL_NAME, 'Bias corrected', UNIT, 5, LONS, LATS, ENS_NUM, LEAD_FINAL, SDATE, dates)
