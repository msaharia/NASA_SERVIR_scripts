...
MONTH_NAME = '{}01'.format(calendar.month_abbr[INIT_FCST_MON].lower())
...
INFILE_template = '{}/RAW_GEOS5.V2/{}/{:04d}/{}/ens{:01d}/geosgcm_vis2d/{}.geosgcm_vis2d.monthly.{:04d}{:02d}.nc4'
#
...
if (VAR=='PRECTOT'):
	...
	MASK_FILE = str(sys.argv[15])
        
LATS = read_nc_files(MASK_FILE, 'lat'); LONS = read_nc_files(MASK_FILE, 'lon');

DATA_DIR = str(sys.argv[17])
MODEL_NAME =  str(sys.argv[16])
...
for LEAD_NUM in range(0, LEAD_FINAL): ## Loop from lead =0 to Final Lead
	...
	for ens in range(ENS_NUM):
		for INIT_FCST_YEAR in range(CLIM_SYR, CLIM_EYR+1):
			...
			if (VAR=='PRECTOT'):
				INFILE = INFILE_template.format(INDIR, '/PRECTOT_Monthly', INIT_FCST_YEAR, MONTH_NAME, ens+1, MONTH_NAME, FCST_YEAR, FCST_MONTH)
			else:
				INFILE = INFILE_template.format(INDIR, '/Monthly', INIT_FCST_YEAR, MONTH_NAME, ens+1, MONTH_NAME, FCST_YEAR, FCST_MONTH)
			print (INFILE)
